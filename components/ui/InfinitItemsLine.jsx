...
function InfinitItemsLine({children , pauseAnimate}) {
  ...
  return (
    <div className='relative w-full p-4 overflow-hidden'>
        <span className='absolute inset-y-0 left-0 z-20 w-[10%] bg-gradient-to-r from-background to-transparent h-full'></span>
            <div className={`relative w-full p-4 flex justify-center gap-x-5 transition-all duration-500
              ${pauseAnimate === true ? "paused" : `${currentLocale == "ar" ? "infinite-line-animate-ar" :"infinite-line-animate"}`}
             `}>
                {children}
            </div>
